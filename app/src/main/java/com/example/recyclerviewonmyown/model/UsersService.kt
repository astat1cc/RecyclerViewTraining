package com.example.recyclerviewonmyown.model

import com.github.javafaker.Faker
import java.util.*
import kotlin.random.Random

typealias UsersListener = (MutableList<User>) -> Unit
...
class UsersService {
        ...
        users.addAll(
            (1..100).map {
                User(
                    ...
                    photo = IMAGES[Random.nextInt(10)],
                    details = faker.lorem().paragraphs(2).joinToString("\n\n")
                )
            }
        )
    }

    fun getById(id: Int) = users.first { it.id == id }
    ...
    fun moveUser(user: User, moveBy: Int) {
        ...
        if (newIndex !in (0..users.size)) return
